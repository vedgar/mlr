...
bound (Atomic _) = Data.Set.empty
bound (Compound _ fs) = unions (map bound fs)
bound (Quantified _ v f) = Data.Set.insert v (bound f)
tally f = (free f, bound f)

test1 = tally (p[x] & fa x (q[f[x1,y],x]))

pNeg (Atomic f) = ne (Atomic f)
pNeg (Compound c [f]) | c == not_c = f
                      | otherwise = Compound (neg_op c) [f]
pNeg (Compound c [f,g]) | c == and_c = pNeg f ||| pNeg g
                        | c == or_c = pNeg f & pNeg g
                        | c == cond_c = f & pNeg g
                        | c == bicond_c = pNeg f <-> g
...
neg_quant Existential = Universal
